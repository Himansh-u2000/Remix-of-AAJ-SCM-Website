...
// Pinned logo for Gurgaon — Eupheus appears in every cycle
import eupheusLogo from "@/assets/clients-ghaziabad/Eupheus.png";

// Recycled logos from other warehouse pages
import harperCollinsLogo from "@/assets/clients-kundli/HarperCollins.png";
...
import nextEducationLogo from "@/assets/clients-sonipat/NextEducation.png";
...
import bezomeLogo from "@/assets/clients-bhiwandi/Bezome.png";
import NetworkPresenceSection from "@/components/NetworkPresenceSection";
import gurgaonFacility from "@/assets/warehouses/gurgaon-facility.webp";

const gurgaonPointers: string[] = [
  'Our Gurgaon facility developed as a <span class="font-semibold text-primary">future-ready</span>, <span class="font-semibold text-primary">technology-led</span> distribution center designed for <span class="font-semibold text-primary">high-scale</span> warehousing and fulfilment operations.',
  'Equipped with a <span class="font-semibold text-primary">21-meter vertical storage</span> capability and <span class="font-semibold text-primary">advanced G+15 racking system</span>, enabling superior space optimization and operational efficiency.',
  'Significant investments made in technology, automation, infrastructure and process capabilities to support <span class="font-semibold text-primary">scalable operations</span> and <span class="font-semibold text-primary">faster throughput</span> management.',
];

const trustBar = [
  { label: "1,00,000+ Sq. Ft." },
...
const whyGurgaon = [
  { title: "Regional Connectivity", desc: "Reach Delhi NCR and major North Indian markets with ease.", icon: Compass },
  { title: "Rapid Fulfilment", desc: "Support same-day & next-day deliveries through strong transport infrastructure.", icon: Zap },
  { title: "Optimized Inventory Movement", desc: "Reduce delays and improve stock movement efficiency.", icon: Workflow },
  { title: "Scalable Operations", desc: "Expand distribution capabilities without operational bottlenecks.", icon: Network },
];

const corridors = [
  { code: "NH-48", route: "Gurgaon – Jaipur – Ahmedabad", desc: "A major freight corridor connecting Haryana with Rajasthan and Gujarat, supporting high-frequency regional deliveries and industrial movement." },
  { code: "NH-44", route: "Gurgaon – Panipat – Chandigarh", desc: "India's longest national highway connects Gurgaon with major North Indian industrial and consumption markets for faster interstate distribution." },
  { code: "Dwarka Expressway", route: "Gurgaon – Delhi Connectivity", desc: "A high-speed corridor improving freight movement between Gurgaon and Delhi NCR while reducing urban congestion and transit delays." },
  { code: "KMP Expressway", route: "Kundli – Manesar – Palwal", desc: "Critical freight bypass route connecting Gurgaon with NCR industrial zones and long-haul transport corridors without inner-city routing." },
];

const industrialZones = ["Manesar", "Udyog Vihar", "Bilaspur", "Sohna Road", "Pataudi Road", "Kadipur"];
const logisticsCorridors = ["IMT Manesar", "Bhiwadi Corridor", "Dwarka Expressway", "NH-48 Belt", "Farukhnagar", "Tauru Road"];

const facilityStats = [
  { value: "1L+", unit: "Sq. Ft.", label: "Strategically located in Gurgaon", icon: Warehouse },
...
const builtFor = [
  { tag: "Quick Commerce", title: "Quick commerce and hyperlocal delivery brands", desc: "Operations that require sub-hour or same-day dispatch need a warehouse strategically located within Gurgaon's core delivery network.", icon: Zap },
  { tag: "Marketplaces", title: "Marketplace sellers with strong Gurgaon order flow", desc: "Amazon, Flipkart, Meesho, Myntra orders concentrated in North India zones.", icon: Box },
  ...
  { tag: "Scaling", title: "Growing brands handling rising order volume in Gurgaon", desc: "Increasing demand that needs closer inventory placement for stability.", icon: Rocket },
];
...
const operations = [
  { title: "Planned Dispatch Cycles", desc: "In a high-volume market like Gurgaon, dispatch needs to follow fixed cycles to meet same-day, next-day and quick commerce SLAs without delays or congestion." },
  { title: "Movement-Level Inventory Tracking", desc: "With faster inventory rotation across Gurgaon, tracking movement at every step ensures stock accuracy during high-frequency order processing." },
  ...
  { title: "Multi-Distributor Order Structuring", desc: "Serving multiple regions from Gurgaon requires structured order handling for different distributors, cities and shipment types." },
  { title: "Transport-Aligned Dispatch Planning", desc: "Gurgaon's dense transport network allows faster movement, but only when dispatch is aligned with route availability and carrier schedules." },
];
...
const migrationSteps = [
  ...
  { title: "Process Alignment", desc: "Workflows are aligned to match how orders, inventory and dispatch will run from the Gurgaon warehouse." },
...
const faqs = [
  { q: "How does warehousing in Gurgaon help brands reduce operational dependency on multiple NCR storage locations?", a: "Centralised inventory positioning in Gurgaon helps streamline fulfilment operations, improve stock control and reduce operational fragmentation across NCR markets." },
  { q: "Can your Gurgaon warehouse support high-frequency replenishment for modern trade and retail networks?", a: "Yes. The warehouse supports scheduled replenishment cycles and recurring dispatch operations for retail chains, distributors and regional channel partners." },
  { q: "How do you manage inventory flow for brands experiencing unpredictable order patterns?", a: "Warehouse workflows are designed to adapt to fluctuating order volumes through flexible storage allocation, manpower planning and dispatch coordination." },
  { q: "Can brands use your Gurgaon warehouse for pre-positioning inventory before major online sales events?", a: "Yes. Inventory can be inwarded and strategically positioned in advance to support faster order processing during large-scale ecommerce campaigns and flash sales." },
  { q: "How does AAJ Gurgaon improve operational efficiency for brands handling large regional dispatch volumes daily?", a: "Operations are optimised through structured inventory movement, organised dispatch workflows and transport-aligned shipment planning for high-volume fulfilment." },
  { q: "Can your Gurgaon warehouse support route-wise shipment segregation for faster North India distribution?", a: "Yes. Orders can be segregated based on destination routes and regional delivery zones to improve loading efficiency and transit movement." },
  { q: "How do you minimise warehouse processing delays during simultaneous inbound and outbound operations?", a: "Inbound and outbound activities are managed through planned dock scheduling, organised workflows and movement coordination to maintain operational continuity." },
  { q: "Can your Gurgaon warehouse support brands looking to improve delivery performance in high-density urban markets?", a: "Yes. Gurgaon's strong connectivity to NCR consumption zones helps brands improve fulfilment speed and delivery timelines across densely populated urban markets." },
];

const Gurgaon = () => {
  return (
    <>
      <Seo {...pageSeo["/warehouses/gurgaon"]} />
          ...
          <div className="mt-6 grid gap-12 lg:grid-cols-12 lg:gap-10">
            <div className="lg:col-span-7">
              <div className="inline-flex items-center gap-2 rounded-full border border-primary/20 bg-primary-soft px-3 py-1 text-xs font-semibold uppercase tracking-[0.18em] text-primary">
                <MapPin className="h-3.5 w-3.5" /> Gurgaon · Haryana, NCR
              </div>
              <h1 className="mt-5 font-display text-4xl font-semibold leading-[1.05] md:text-5xl lg:text-[56px]">
                Power Faster Fulfilment with{" "}
                <span className="text-primary">Advanced Warehouse Services</span>{" "}
                in Gurgaon
              </h1>
              <p className="mt-6 max-w-2xl text-lg leading-relaxed text-muted-foreground">
                Streamline your warehouse operations in Gurgaon with faster inventory movement, accurate order processing and scalable fulfilment capabilities built for growing business demand.
              </p>
              <p className="mt-4 max-w-2xl text-sm text-muted-foreground">
                Trusted warehouse company in Gurgaon managing high-volume inventory, rapid dispatch cycles and complex fulfilment operations with operational precision.
              </p>
            ...
            <div className="lg:col-span-5">
              <div className="relative overflow-hidden rounded-3xl border border-border bg-card p-7 shadow-elevated md:p-8">
                ...
                <div className="relative flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.18em] text-primary">
                  <Sparkles className="h-3.5 w-3.5" /> Gurgaon Hub Snapshot
                </div>
                ...
                <div className="relative mt-6">
                  ...
                  <div className="mt-7 rounded-2xl border border-border bg-secondary/40 p-4">
                    <div className="mb-3 flex items-center justify-between text-[11px] font-semibold uppercase tracking-wider text-muted-foreground">
                      <span>Delhi</span><span>Gurgaon</span><span>Jaipur</span>
                    </div>
                  ...
                  <div className="mt-5 flex items-start gap-3 rounded-xl bg-primary/5 p-4 text-sm text-foreground">
                    ...
                    <span>Positioned along NH-48 and the KMP Expressway — Delhi NCR, Rajasthan and Western markets within easy reach.</span>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div className="container mx-auto px-6">
          <div className="mx-auto max-w-3xl text-center">
            <SectionHeader
              align="center"
              className="mx-auto"
              title={<>Gurgaon a Major Milestone in our <span className="text-primary">Fulfilment Journey</span></>}
            />
          </div>

          <div className="mt-10 grid items-stretch gap-6 lg:grid-cols-12 lg:gap-8">
            {/* Left: 3 pointer cards on checkered/grid background */}
            <div className="relative overflow-hidden rounded-3xl border border-border bg-card p-6 shadow-card md:p-8 lg:col-span-6">
              <div aria-hidden className="bg-grid pointer-events-none absolute inset-0 opacity-70" />
              <ul className="relative space-y-4">
                {gurgaonPointers.map((p, i) => (
                  <li
                    key={i}
                    className="group rounded-2xl border border-primary/15 bg-primary-soft/60 p-5 backdrop-blur-sm transition hover:-translate-y-0.5 hover:border-primary/40 hover:shadow-elevated md:p-6"
                  >
                    <div className="flex items-start gap-5">
                      <div className="font-display text-3xl font-bold leading-none text-primary md:text-4xl">
                        0{i + 1}
                      </div>
                      <p
            ...
            {/* Right: Large warehouse image card */}
            <div className="relative overflow-hidden rounded-3xl border border-border bg-card shadow-card lg:col-span-6">
              <div className="relative aspect-[4/3] w-full overflow-hidden lg:aspect-auto lg:h-full lg:min-h-[420px]">
                <img
                  src={gurgaonFacility}
                  alt="AAJ Gurgaon Warehouse — India's Tallest Warehouse"
                  loading="lazy"
                  width={1400}
                  height={1050}
                  className="h-full w-full object-cover"
                />
                <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/65 via-black/30 to-transparent p-5 md:p-6">
                  <div className="flex items-center gap-2 text-white">
                    <span className="inline-block h-2.5 w-2.5 rounded-full bg-primary shadow-[0_0_0_4px_hsl(var(--primary)/0.25)]" />
                    <span className="font-display text-base font-semibold md:text-lg">
                      Gurgaon Warehouse
                    </span>
                    <span className="text-sm italic text-white/85 md:text-base">
      ...
      <section className="relative overflow-hidden bg-secondary/40 py-11 md:py-14">
        <div className="container mx-auto px-6">
          <SectionHeader
            ...
            title={<>How Gurgaon's Transport Network Moves Your Inventory Faster <span className="text-primary">Across North India</span></>}
            description="Gurgaon sits at the centre of North India's most important freight and industrial corridors. Its highway connectivity gives businesses faster access to Delhi NCR, Rajasthan, Punjab, Uttar Pradesh and other high-demand consumption markets — built around rapid freight movement and large-scale distribution."
            align="center"
            className="mx-auto"
          />
        ...
        <div className="container relative mx-auto px-6">
          <div className="max-w-3xl">
            <SectionHeader
              ...
              title={<>Gurgaon's Industrial Ecosystem and <span className="text-primary">What It Means for Your Inventory</span></>}
              description="Manesar, Udyog Vihar and Bilaspur are densely connected with transporters, courier hubs, packaging vendors and industrial clusters — enabling faster inbound freight, shorter carrier pickup windows and better access to multiple logistics partners."
            />
          </div>
            ...
            <div className="rounded-3xl border border-border bg-card p-7 shadow-card">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <h3 className="font-display text-lg font-semibold text-ink">Gurgaon Clusters We Operate Near</h3>
                </div>
              </div>
      ...
      <section id="facility" className="bg-secondary/40 py-11 md:py-14">
        <div className="container mx-auto px-6">
          <SectionHeader
            ...
            title={<>A Warehouse Facility in Gurgaon That Supports <span className="text-primary">Faster Movement Across North India</span></>}
            align="center"
            className="mx-auto"
          />
            ...
            <div className="grid lg:grid-cols-12">
              <div className="lg:col-span-5">
                <div className="aspect-[4/3] w-full bg-secondary lg:h-full">
                  <iframe
                    title="AAJ Supply Chain Management – Gurgaon"
                    src="https://maps.google.com/maps?q=28.2410101,76.9350235&z=17&output=embed"
              ...
              <div className="p-7 md:p-10 lg:col-span-7">
                <div className="inline-flex items-center gap-2 rounded-full bg-primary/10 px-3 py-1 text-xs font-semibold uppercase tracking-[0.18em] text-primary">
                  <Building2 className="h-3.5 w-3.5" /> Gurgaon, Haryana
                </div>
                <h3 className="mt-4 font-display text-2xl font-semibold text-ink">AAJ Supply Chain Management – Gurgaon</h3>
                <p className="mt-3 flex gap-2 text-sm text-muted-foreground">
                  <MapPin className="mt-0.5 h-4 w-4 shrink-0 text-primary" />
                  <span>Jhamuwas (Hadbast No. 31) and Gudhi (Hadbast No. 29), Tehsil: Tauru, District: Nuh, Haryana</span>
                </p>
                ...
                <div className="mt-7 flex flex-wrap gap-3">
                  <Button asChild variant="outline" size="sm">
                    <a href="https://maps.app.goo.gl/g3Xrg6ag5YqTwAff6" target="_blank" rel="noreferrer">
      ...
      <section className="bg-background py-11 md:py-14">
        <div className="container mx-auto px-6">
          <SectionHeader
            ...
            title={<>Operations We Run at Our <span className="text-primary">Gurgaon Warehouse</span></>}
            align="center"
            className="mx-auto"
          />
      ...
      <section className="bg-secondary/40 py-11 md:py-14">
        <div className="container mx-auto px-6">
          <SectionHeader
            ...
            description="If your operations involve North India demand, faster delivery expectations or rising order volume, AAJ's warehousing company in Gurgaon is built to support that shift."
            align="center"
            className="mx-auto"
          />
      ...
      <section className="bg-secondary/40 py-11 md:py-14">
        <div className="container mx-auto px-6">
          <SectionHeader
            eyebrow="How AAJ runs Gurgaon"
            title={<>Operational Practices That <span className="text-primary">Scale with Your Volume</span></>}
            description="These are the operational principles that keep our warehouse services in Gurgaon reliable as your volume and SKU count grow."
            align="center"
            className="mx-auto"
          />
      ...
      <section className="bg-background py-11 md:py-14">
        <div className="container mx-auto px-6">
          <SectionHeader
            ...
            title={<>Move Operations to Our Gurgaon Warehouse <span className="text-primary">Without Disrupting Orders</span></>}
            align="center"
            className="mx-auto"
          />
          <div className="relative mt-14">
      ...
      <section className="bg-secondary/40 py-11 md:py-14">
        <div className="container mx-auto px-6">
          <SectionHeader
            ...
            title={<>Hear from Brands Shipping Out of <span className="text-primary">Our Gurgaon Hub</span></>}
            align="center"
            className="mx-auto"
          />
          <div className="mt-12 rounded-2xl border border-dashed border-border bg-card p-10 text-center text-sm text-muted-foreground">
            ...
            <p className="mt-3">Client testimonials from brands shipping out of Gurgaon will appear here soon.</p>
          </div>
        </div>
      </section>
      )}
      ...
      <NetworkPresenceSection currentSlug="gurgaon" />
      ...
      <section className="bg-background py-11 md:py-14">
        <div className="container mx-auto px-6">
          <SectionHeader
            eyebrow="FAQs"
            title={<>Quick Answers About <span className="text-primary">Our Gurgaon Warehousing</span></>}
            align="center"
            className="mx-auto"
          />
            ...
            <div className="relative grid gap-8 md:grid-cols-12 md:items-center">
              <div className="md:col-span-8">
                ...
                <p className="mt-3 max-w-2xl text-base text-muted-foreground">
                  We review inventory, dispatch, coordination and same-day fulfilment readiness — then show you what shifting to Gurgaon could change.
                </p>
              </div>
...
/* =========== CLIENT LOGOS — Gurgaon rotation
   Eupheus is pinned and appears in EVERY cycle, but its slot position
   randomises over time. The remaining 5 slots cycle through a pool of
   recycled logos from other warehouse pages. ============================ */
const ClientLogos = () => {
  const eupheus = { src: eupheusLogo, alt: "Eupheus Learning" };

  const pool: { src: string; alt: string }[] = [
    { src: harperCollinsLogo, alt: "HarperCollins" },
    ...
    { src: simonLogo, alt: "Simon & Schuster" },
  ...
  // poolIndices[slot] is index into pool (ignored for the eupheus slot)
  const [poolIndices, setPoolIndices] = useState<number[]>(() =>
    Array.from({ length: SLOTS }, (_, i) => i),
  );
  const [eupheusSlot, setEupheusSlot] = useState<number>(0);
  const nextPoolRef = useRef<number>(SLOTS);

  const handleIter = (slotIdx: number) => {
    if (slotIdx === eupheusSlot) {
      // Move Eupheus to a different random slot, and fill old slot with a fresh logo
      const candidates = Array.from({ length: SLOTS }, (_, i) => i).filter((i) => i !== eupheusSlot);
      const newSlot = candidates[Math.floor(Math.random() * candidates.length)];
      setPoolIndices((prev) => {
        const next = [...prev];
        next[slotIdx] = nextPoolRef.current % pool.length;
        nextPoolRef.current += 1;
        return next;
      });
      setEupheusSlot(newSlot);
    } else {
      setPoolIndices((prev) => {
        const next = [...prev];
        next[slotIdx] = nextPoolRef.current % pool.length;
      ...
      <div className="container relative">
        <div className="grid grid-cols-3 gap-x-6 gap-y-8 md:grid-cols-6 md:gap-8">
          {Array.from({ length: SLOTS }).map((_, slotIdx) => {
            const isEupheus = slotIdx === eupheusSlot;
            const logo = isEupheus ? eupheus : pool[poolIndices[slotIdx]];
            const animKey = isEupheus ? `e-${slotIdx}` : `p-${poolIndices[slotIdx]}-${slotIdx}`;
            return (
              <div
                key={slotIdx}
